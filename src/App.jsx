...
function SplashScreen({ onDone, config, photos = [] }) {
  ...
  // Cycle photos every 560ms — shows all 5 within the 3s window
  useEffect(()=>{
    if (splashPhotos.length <= 1) return;
    const iv = setInterval(()=>setPhotoIdx(i=>(i+1)%splashPhotos.length), 560);
        ...
        <>
          {splashPhotos.map((src,i)=>(
            <div key={i} style={{
              ...
              opacity:i===photoIdx?0.32:0,
              transition:`opacity 0.7s ease, transform ${i===photoIdx?"6s":"0s"} ease`,
              transform:i===photoIdx?"scale(1.06)":"scale(1)",
              filter:"saturate(1.1) brightness(0.95)",
            }}/>
          ))}
          {/* White gradient overlay — keeps center crisp, photos visible at edges */}
          <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at center, rgba(255,255,255,0.82) 0%, rgba(255,255,255,0.55) 60%, rgba(255,255,255,0.3) 100%)"}}/>
          {/* Photo counter dots */}
          {splashPhotos.length>1&&(
            <div style={{position:"absolute",bottom:"4rem",left:"50%",transform:"translateX(-50%)",display:"flex",gap:"5px",zIndex:2}}>
              {splashPhotos.map((_,i)=>(
                <div key={i} style={{width:i===photoIdx?"18px":"5px",height:"5px",borderRadius:"3px",background:i===photoIdx?C.primary:"rgba(26,26,46,0.2)",transition:"all 0.4s ease"}}/>
              ))}
            </div>
          )}
        </>
...
function EmergencySOS({ listing }) {
  ...
  // Don't auto-locate on open — wait for user to tap the locate button
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div style={{position:"fixed",inset:0,zIndex:99999,background:"rgba(20,10,10,0.55)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"80px 1rem 1rem",overflowY:"auto"}} onClick={e=>e.target===e.currentTarget&&setOpen(false)}>
          <div style={{background:"#FDFAF5",borderRadius:"12px",width:"100%",maxWidth:"540px",margin:"1rem 0 3rem",boxShadow:"0 32px 80px rgba(0,0,0,0.5)",overflow:"hidden"}}>
            ...
            <div style={{padding:"1.2rem 1.4rem"}}>
              ...
              {listing && (listing.nearestHospitalName || listing.nearestPoliceName) && (
                <div style={{marginBottom:"1.2rem"}}>
                  ...
                  {listing.nearestHospitalName && (
                    <div style={cardStyle}>
                      <div style={{fontSize:"0.85rem",fontWeight:600,color:C.cream}}>🏥 {listing.nearestHospitalName}</div>
                  ...
                  {listing.nearestPoliceName && (
                    <div style={cardStyle}>
                      <div style={{fontSize:"0.85rem",fontWeight:600,color:C.cream}}>🚓 {listing.nearestPoliceName}</div>
              ...
              {geoStatus==="idle" && (
                <button onClick={locate} style={{width:"100%",padding:"0.85rem",background:C.teal,color:"#fff",border:"none",borderRadius:"8px",fontSize:"0.85rem",fontWeight:700,cursor:"pointer",marginBottom:"0.8rem",display:"flex",alignItems:"center",justifyContent:"center",gap:"0.5rem"}}>
                ...
                <div>
                  {results.hospitals.slice(0,2).map((h,i)=>(
                    <div key={i} style={cardStyle}>
                      <div style={{fontSize:"0.85rem",fontWeight:600,color:C.cream}}>🏥 {h.name}</div>
                  ...
                  {results.police.slice(0,2).map((p,i)=>(
                    <div key={i} style={cardStyle}>
                      <div style={{fontSize:"0.85rem",fontWeight:600,color:C.cream}}>🚓 {p.name}</div>
        ...
        {step==="success"&&(
          <div style={{animation:"fadeIn 0.4s ease"}}>
            <div style={{textAlign:"center",marginBottom:"1.5rem"}}>
              ...
              <div style={{fontFamily:"'Fraunces',Georgia,serif",fontSize:"1.3rem",color:C.cream,marginBottom:"0.4rem"}}>{isDeposit?"Dates Held!":"Booking Confirmed!"}</div>
            ...
            {isDeposit&&<div style={{padding:"0.7rem 1rem",background:"rgba(255,107,107,0.06)",border:"1px solid rgba(255,107,107,0.2)",borderRadius:"6px",marginBottom:"1rem",fontSize:"0.75rem",color:C.muted}}>
              💰 Balance due at check-in: <strong style={{color:C.primary}}>KES {fmt(balanceDue)}</strong>
            </div>}

            {/* Bolt ride button — show on success so guest can get to the property */}
            {listing.lat && listing.lng && (
              <div style={{marginBottom:"0.8rem",padding:"1rem",background:`linear-gradient(135deg,${C.tealDim},rgba(78,205,196,0.06))`,border:`1px solid ${C.tealDim}`,borderRadius:"8px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"0.8rem",flexWrap:"wrap"}}>
                <div>
                  <div style={{fontSize:"0.72rem",fontWeight:600,color:C.teal,marginBottom:"0.15rem"}}>Need a ride to your stay?</div>
                  <div style={{fontSize:"0.7rem",color:C.muted}}>We'll open Bolt with {listing.name} as your destination.</div>
                </div>
                <button onClick={()=>openBoltRide({destLat:listing.lat,destLng:listing.lng,destAddress:`${listing.name}, ${listing.neighborhood}`})}
                  style={{background:"#34D186",color:"#fff",border:"none",borderRadius:"6px",padding:"0.6rem 1.1rem",fontSize:"0.76rem",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                  🚗 Book with Bolt
                </button>
              </div>
            )}
      ...
      <div style={box}>
        ...
        {step==="success"&&(
          <div style={{animation:"fadeIn 0.4s ease",textAlign:"center"}}>
            ...
            <div style={{fontFamily:"'Fraunces',Georgia,serif",fontSize:"1.4rem",color:C.cream,marginBottom:"0.4rem"}}>Balance Cleared! 🎉</div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#FDFAF5",display:"flex",paddingTop:"72px"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"1.5rem",animation:"fadeIn 0.3s ease",background:"#FDFAF5"}}>
        ...
        {tab==="settings"&&<SettingsPanel onLogout={handleLogout} hostProfile={hostProfile} onProfileSaved={setHostProfile}/>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <>
      ...
      <Nav onNavigate={navigate} listing={page==="listing"?selectedListing:null}/>
