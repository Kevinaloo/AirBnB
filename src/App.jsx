        ...
        <>
          {splashPhotos.map((src,i)=>(
            <div key={i} style={{
              ...
              opacity:i===photoIdx?0.18:0,
              transition:"opacity 0.7s ease",
              transform:i===photoIdx?"scale(1.04)":"scale(1)",
              filter:"saturate(1.1) brightness(0.95)",
              transitionProperty:"opacity, transform",
              transitionDuration:"0.7s, 4s",
            }}/>
          ))}
          {/* White gradient overlay — keeps text crisp */}
          <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at center, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.95) 65%, #ffffff 100%)"}}/>
          {/* Photo counter dots */}
...
function EmergencySOS({ listing }) {
  ...
  return (
    <>
      ...
      {open && (
        <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(20,10,10,0.55)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"80px 1rem 1rem",overflowY:"auto"}} onClick={e=>e.target===e.currentTarget&&setOpen(false)}>
